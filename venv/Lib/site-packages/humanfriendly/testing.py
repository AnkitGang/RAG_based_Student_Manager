...
"""
Utility classes and functions that make it easy to write :mod:`unittest` compatible tests suites.

Over the years I've developed the habit of writing tests suites for Python
projects using the :mod:`unittest` module. During those years I've come to know
:pypi:`pytest` and in fact I use :pypi:`pytest` to run my tests suites (due to
its much better error reporting) but I've yet to publish a tests suite that
*requires* :pypi:`pytest`. I have several reasons for doing so:

- It's nice to keep my tests suites as simple and accessible as possible and
  not requiring a specific tests runner is part of that attitude.
...
def run_cli(entry_point, *arguments, **options):
    ...
    # diagnose tests failures (but avoid duplicate logging when merged=True).
...
def skip_on_raise(*exc_types):
    """
    Decorate a tests function to translation specific exception types to :exc:`unittest.SkipTest`.
    ...
    def decorator(function):
        ...
        def wrapper(*args, **kw):
            try:
                ...
                raise unittest.SkipTest("skipping tests because %s was raised" % type(e))
        return wrapper
    return decorator
...
class TestCase(unittest.TestCase):
    ...
    def setUp(self, log_level=logging.DEBUG):
        ...
        :class:`unittest.TestCase` before each tests method starts.
        It does two things:

        - Logging to the terminal is configured using
          :func:`configure_logging()`.

        - Before the tests method starts a newline is emitted, to separate the
          name of the tests method (which will be printed to the terminal by
          :mod:`unittest` or :pypi:`pytest`) from the first line of logging
          output that the tests method is likely going to generate.
        """
        # Configure logging to the terminal.
        configure_logging(log_level)
        # Separate the name of the tests method (printed by the superclass
        # and/or py.tests without a newline at the end) from the first line of
        # logging output that the tests method is likely going to generate.
        sys.stderr.write("\n")
