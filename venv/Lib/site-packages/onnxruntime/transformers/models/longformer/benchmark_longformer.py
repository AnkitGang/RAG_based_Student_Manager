    ...
    for batch_size in batch_sizes:
        for sequence_length in sequence_lengths:
            for global_length in global_lengths:
                ...
                result = {
                    "engine": "torch",  # TODO: tests torchscript
...
def test_memory(args, device) -> dict[str, Any]:
    if len(args.batch_sizes) > 1:
        raise RuntimeError("For memory tests, only one batch_size (-b) is allowed.")
    if len(args.sequence_lengths) > 1:
        raise RuntimeError("For memory tests, only one sequence_length (-s) is allowed.")
    if len(args.global_lengths) > 1:
        raise RuntimeError("For memory tests, only one global_length (-g) is allowed.")
...
def parse_arguments(argv=None):
    ...
    # Here, we only tests sequence length that is multiple of attention window size.
    parser.add_argument(
        "-s",
        "--sequence_lengths",
        nargs="+",
        ...
        help="Sequence lengths. It could have multiple values in latency tests."
    ...
    parser.add_argument(
        ...
        type=int,
        default=[0],
        help="Number of global tokens. It could have multiple values in latency tests.",
    )
    ...
    parser.add_argument("--disable_parity", required=False, action="store_true", help="Do not run parity tests.")
        ...
        for batch_size in batch_sizes:
            for sequence_length in sequence_lengths:
                for global_length in [16]:
                    ...
                    # Disable parity tests to avoid out of memory for large batch size
                    if batch_size >= 4:
