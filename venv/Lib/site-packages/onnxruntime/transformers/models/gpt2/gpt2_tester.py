...
class Gpt2Tester:
    ...
    def save_test_data(self, session, output, save_test_data_dir, test_case_id):
        ...
        if os.path.exists(path):
            print(f"Directory {path} existed. Skip saving tests data")
            return
    ...
    def test_generation(
        ...
        It will print top 1 and top k errors on the given tests inputs.
        """
        print(
            f"start tests generation: (top_k={top_k} top_k_no_order={top_k_no_order} max_steps={max_steps} test_inputs={len(test_inputs)} max_inputs={max_inputs})"
        )
