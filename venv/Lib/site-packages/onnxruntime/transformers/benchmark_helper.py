    ...
    if use_gpu:
        if provider == "dml":
            assert "DmlExecutionProvider" in onnxruntime.get_available_providers(), (
                "Please install onnxruntime-directml package to tests GPU inference."
            )
            ...
            ), "Please install onnxruntime-gpu package, or install ROCm support, to tests GPU inference."
...
def allocateOutputBuffers(output_buffers, output_buffer_max_sizes, device):  # noqa: N802
    # Allocate output tensors with the largest tests size needed. So the allocated memory can be reused
    # for each tests run.
