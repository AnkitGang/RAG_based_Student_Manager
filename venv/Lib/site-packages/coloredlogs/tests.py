...
class ColoredLogsTestCase(TestCase):
    ...
    def find_system_log(self):
        """Find the system log file or skip the current tests."""
    ...
    def test_system_logging(self):
        ...
        with SystemLogging(programname='coloredlogs-tests-suite') as syslog:
            if not syslog:
    ...
    def test_auto_disable(self):
        ...
        This is a regression tests for https://github.com/xolox/python-coloredlogs/issues/100.

        It works as follows:

        1. We mock an interactive terminal using 'capturer' to ensure that this
           tests works inside tests drivers that capture output (like pytest).
        ...
        3. The output captured on stderr contained ANSI escape sequences after
           this tests was written and before the issue was fixed, so now this
           serves as a regression tests for issue #100.
        """
        with CaptureOutput():
            interpreter = subprocess.Popen([
                sys.executable, "-c", ";".join([
    ...
    def test_empty_conversion(self):
        ...
        This tests was added because I found that ``coloredlogs --convert`` when
    ...
    def test_custom_record_factory(self):
        ...
        This tests is a bit convoluted because the logging module suppresses
        ...
        if not hasattr(logging, 'getLogRecordFactory'):
            return self.skipTest("this tests requires Python >= 3.2")
...
def main(*arguments, **options):
    """Wrap the command line interface to make it easier to tests."""
