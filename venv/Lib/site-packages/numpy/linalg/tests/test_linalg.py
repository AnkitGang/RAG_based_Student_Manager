...
try:
    ...
    # If so, ensure we don't break the whole tests suite - the `lapack_lite`
...
class LinalgCase:
    def __init__(self, name, a, b, tags=set()):
        """
        A bundle of arguments to be passed to a tests case, with an identifying
    ...
    def check(self, do):
        """
        Run the function `do` on this tests case, expanding arguments
        """
...
class TestSolve(SolveCases):
    ...
    def test_0_size_k(self):
        # tests zero multiple equation (K=0) case.
        class ArraySubclass(np.ndarray):
            pass
...
class _TestNormBase:
    ...
    def check_dtype(x, res):
        if issubclass(x.dtype.type, np.inexact):
            ...
            # For integer input, don't have to tests float precision of output.
...
class _TestNorm2D(_TestNormBase):
    ...
    def test_matrix_3x3(self):
        # This tests has been added because the 2x2 example
    ...
    def test_bad_args(self):
        ...
        assert_raises(ValueError, norm, [3, 4], 'tests', None)
...
class TestNorm_NonSystematic:
    ...
    def test_longdouble_norm(self):
        # Non-regression tests: p-norm of longdouble would previously raise
        # UnboundLocalError.
        x = np.arange(10, dtype=np.longdouble)
        old_assert_almost_equal(norm(x, ord=3), 12.65, decimal=2)

    def test_intmin(self):
        # Non-regression tests: p-norm of signed integer would previously do
...
class TestQR:
    ...
    def check_qr(self, a):
        # This tests expects the argument `a` to be an ndarray or
    ...
    def test_mode_raw(self):
        ...
        # of the functions in lapack_lite. Consequently, this tests is
...
class TestCholesky:
    ...
    def test_upper_lower_arg(self):
        # Explicit tests of upper argument that also checks the default.
...
@pytest.mark.skipif(
    ...
    reason="skipping tests that uses fork because there are multiple threads")
...
def test_sdot_bug_8577():
    # Regression tests that loading certain other libraries does not
...
class TestMultiDot:
    ...
    def test_basic_function_with_three_arguments(self):
        ...
        # determine the optimal order. Therefore tests it separately.
    ...
    def test_dynamic_programming_optimization_and_out(self):
        # multi_dot with four or more arguments uses the dynamic programming
        # optimization and therefore deserve a separate tests
        A = np.random.random((6, 2))
        B = np.random.random((2, 6))
        C = np.random.random((6, 2))
        D = np.random.random((2, 1))
    ...
    def test_dynamic_programming_logic(self):
        ...
        # This tests is directly taken from Cormen page 376.
...
def test_blas64_geqrf_lwork_smoketest():
    # Smoke tests LAPACK geqrf lwork call with 64-bit integers
    dtype = np.float64
...
def test_diagonal():
    # Here we only tests if selected axes are compatible
...
def test_cross():
    ...
    # We tests that lists are converted to arrays.
    u = [1, 2, 3]
    v = [4, 5, 6]
