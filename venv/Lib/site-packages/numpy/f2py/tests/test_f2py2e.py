...
def test_gh23598_warn(capfd, gh23598_warn, monkeypatch):
    foutl = get_io_paths(gh23598_warn, mname="tests")
    ipath = foutl.f90inp
    monkeypatch.setattr(
        sys, "argv",
        f'f2py {ipath} -m tests'.split())
...
def test_f2py_skip(capfd, retreal_f77, monkeypatch):
    ...
    CLI :: skip:
    """
    foutl = get_io_paths(retreal_f77, mname="tests")
    ipath = foutl.finp
    toskip = "t0 t4 t8 sd s8 s4"
    remaining = "td s0"
    monkeypatch.setattr(
        sys, "argv",
        f'f2py {ipath} -m tests skip: {toskip}'.split())
...
def test_f2py_only(capfd, retreal_f77, monkeypatch):
    ...
    monkeypatch.setattr(
        sys, "argv",
        f'f2py {ipath} -m tests only: {tokeep}'.split())
...
def test_lower_cmod(capfd, hello_world_f77, monkeypatch):
    ...
    CLI :: --[no-]lower
    """
    foutl = get_io_paths(hello_world_f77, mname="tests")
    ipath = foutl.finp
    capshi = re.compile(r"HI\(\)")
    capslo = re.compile(r"hi\(\)")
    # Case I: --lower is passed
    monkeypatch.setattr(sys, "argv", f'f2py {ipath} -m tests --lower'.split())
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
    ...
    monkeypatch.setattr(sys, "argv",
                        f'f2py {ipath} -m tests --no-lower'.split())
    with util.switchdir(ipath.parent):
        f2pycli()
        out, _ = capfd.readouterr()
...
def test_lower_sig(capfd, hello_world_f77, monkeypatch):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f'f2py {ipath} -h {foutl.pyf} -m tests --overwrite-signature'.split(),
    )
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        f'f2py {ipath} -h {foutl.pyf} -m tests --overwrite-signature --no-lower'
        .split(),
    )
