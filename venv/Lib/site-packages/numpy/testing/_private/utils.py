...
class KnownFailureException(Exception):
    '''Raise this exception to mark a tests as a known failing tests.'''
    pass
...
def assert_(val, msg=''):
    ...
    """
    __tracebackhide__ = True  # Hide traceback for py.tests
    if not val:
        try:
            smsg = msg()
        except TypeError:
...
def assert_equal(actual, desired, err_msg='', verbose=True, *, strict=False):
    ...
    Here, the tests checks that each value in `x` is 3:
    ...
    # isscalar tests to check cases such as [np.nan] != np.nan
...
def print_assert_equal(test_string, actual, desired):
    """
    Test if two objects are equal, and print an error message if tests fails.

    The tests is performed with ``actual == desired``.
    ...
    actual : object
        The object to tests for equality against `desired`.
    desired : object
        The expected result.
...
def assert_almost_equal(actual, desired, decimal=7, err_msg='', verbose=True):
    ...
    The tests verifies that the elements of `actual` and `desired` satisfy::

        abs(desired-actual) < float64(1.5 * 10**(-decimal))

    That is a looser tests than originally documented, but agrees with what the
    ...
    try:
        ...
        # check that both are nan if any is a nan, and tests for equality
        # otherwise
...
def assert_array_equal(actual, desired, err_msg='', verbose=True, *,
    ...
    handling for scalars. Here the tests checks that each value in `x` is 3:
...
def assert_array_almost_equal(actual, desired, decimal=6, err_msg='',
    ...
    The tests verifies identical shapes and that the elements of ``actual`` and
    ``desired`` satisfy::
...
def assert_array_less(x, y, err_msg='', verbose=True, *, strict=False):
    ...
    assert_array_almost_equal: tests objects for equality up to precision
...
def assert_string_equal(actual, desired):
    ...
    ----------
    actual : str
        The string to tests for equality against the expected string.
    desired : str
        The expected string.
...
def rundocs(filename=None, raise_on_error=True):
    ...
    argument to the ``tests()`` call. For example, to run all tests (including
    doctests) for ``numpy.lib``:

    >>> np.lib.tests(doctests=True)  # doctest: +SKIP
    """
...
def assert_raises(*args, **kwargs):
    ...
    thrown, it will not be caught, and the tests case will be
...
def assert_allclose(actual, desired, rtol=1e-7, atol=0, equal_nan=True,
    ...
    The tests is equivalent to ``allclose(actual, desired, rtol, atol)`` (note
    ...
    handling for scalars. Here, the tests checks that the value of `numpy.sin`
...
def tempdir(*args, **kwargs):
    """Context manager to provide a temporary tests folder.
...
def _assert_no_gc_cycles_context(name=None):
    ...
    # not meaningful to tests if there is no refcounting
    if not HAS_REFCOUNT:
        yield
        return
...
def requires_memory(free_bytes):
    """Decorator to skip a tests if not enough memory is available"""
    import pytest
...
def _no_tracing(func):
    """
    Decorator to temporarily turn off tracing for the duration of a tests.
