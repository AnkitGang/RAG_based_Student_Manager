...
class TestUfunc:
    ...
    def test_signature_dtype_instances_allowed(self, get_kwarg):
        ...
        # Relies on pickling behavior, if assert fails just remove tests...
    ...
    def test_cast_safety(self, ufunc):
        """Basic tests for the safest casts, because ufuncs inner loops can
    ...
    def test_cast_safety_scalar(self, ufunc):
        # We tests add and equal, because equal has special scalar handling
                    ...
                    if tc in 'FDG':
                        ...
                        with suppress_warnings() as sup:
                            ...
                            if not np.isnan(np.finfo(dtout).tiny):
                                ...
                                atol = 3e-308
                        # Some tests values result in invalid for float16
    ...
    def test_object_logical(self):
        a = np.array([3, None, True, False, "tests", ""], dtype=object)
    ...
    def test_where_param_buffer_output(self):
        # This tests is temporarily skipped because it requires
    ...
    @pytest.mark.skipif(sys.maxsize < 2**32,
            reason="tests array too large for 32bit platform")
    def test_identityless_reduction_huge_array(self):
        # Regression tests for gh-20921 (copying identity incorrectly failed)
    ...
    def test_reduce_zero_axis(self):
        ...
        # This tests doesn't actually look at return values, it just checks to
    ...
    def test_logical_ufuncs_support_anything(self, ufunc):
        ...
        # Last check, tests reduction when out and a match (the complexity here
...
def test_ufunc_noncontiguous(ufunc):
    ...
    for typ in ufunc.types:
        ...
        if any(set('O?mM') & set(typ)):
            # bool, object, datetime are too irregular for this simple tests
            continue
        inp, out = typ.split('->')
...
def test_ufunc_warn_with_nan(ufunc):
    # issue gh-15127
    # tests that calling certain ufuncs with a non-standard `nan` value does not
    # emit a warning
...
def test_reduce_casterrors(offset):
    # Test reporting of casting errors in reductions, we tests various
    ...
    with pytest.raises(ValueError, match="invalid literal"):
        ...
        # `initial=None` disables the use of an identity here to tests failures
...
def test_find_access_past_buffer():
    ...
    # this tests will produce an 'Invalid read' if run under valgrind.
...
class TestLowlevelAPIAccess:
    def test_resolve_dtypes_basic(self):
        # Basic tests for dtype resolution:
        i4 = np.dtype("i4")
        f4 = np.dtype("f4")
        f8 = np.dtype("f8")
    ...
    def test_loop_access(self):
        # This is a basic tests for the full strided loop access
