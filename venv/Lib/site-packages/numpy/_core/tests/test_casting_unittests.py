...
The reason is mostly to tests a new implementation of the casting machinery.
...
class TestChanges:
    """
    These tests cases exercise some behaviour changes
    """
...
class TestCasting:
    ...
    def test_simple_direct_casts(self, from_dt):
        """
        This tests checks numeric direct casts for dtypes supported also by the
        struct module (plus complex).  It tries to be tests a wide range of
        ...
        If this tests creates issues, it should possibly just be simplified
        ...
        for to_dt in simple_dtype_instances():
            ...
            if from_res is not from_dt or to_res is not to_dt:
                # Do not tests this case, it is handled in multiple steps,
    ...
    def test_simple_string_casts_roundtrip(self, other_dt, string_char):
        ...
        The tests also covers some string to string casts (but not all).
        ...
        if res_other_dt is not other_dt:
            # do not support non-native byteorder, skip tests in that case
        ...
        assert not cast._supports_unaligned  # if support is added, should tests
    ...
    def test_object_and_simple_resolution(self, dtype):
        # Simple tests to exercise the cast when no instance is specified
        object_dtype = type(np.dtype(object))
    ...
    def test_void_and_structured_with_subarray(self, casting):
        # tests case corresponding to gh-19325
    ...
    def test_structured_view_offsets_parametric(
        ...
        # TODO: While this tests is fairly thorough, right now, it does not
        # really tests some paths that may have nonzero offsets (they don't
        # really exists).
        from_dt = np.dtype(from_dt)
        to_dt = np.dtype(to_dt)
        cast = get_castingimpl(type(from_dt), type(to_dt))
    ...
    def test_object_casts_NULL_None_equivalence(self, dtype):
        ...
        # If the check fails (maybe it should) the tests would lose its purpose:
    ...
    def test_nonstandard_bool_to_other(self, dtype):
        # simple tests for casting bool_ to numeric types, which should not
