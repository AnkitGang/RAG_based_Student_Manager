...
They mainly exists to ensure good tests coverage without having to try the
weirder cases on actual numpy functions but tests them in one place.

The tests function is defined in C to be equivalent to (errors may not always
...
def test_string_fallbacks():
    # We can (currently?) use numpy strings to tests the "slow" fallbacks
