...
class TestArrayRepr:
    ...
    def test_object_subclass(self):
        ...
        # tests that object + subclass is OK:
        x = sub([None, None])
    ...
    def test_void_scalar_recursion(self):
        # gh-9345
        repr(np.void(b'tests'))  # RecursionError ?
...
class TestArray2String:
    ...
    def test_any_text(self, text):
        # This tests checks that, given any value that can be represented in an
...
class TestPrintOptions:
    ...
    def test_0d_arrays(self):
        ...
        assert_equal(repr(np.array('café', '<U4')),
                     "array('café', dtype='<U4')")
        assert_equal(str(np.array('tests', np.str_)), 'tests')
        ...
        x = np.array(1)
        np.set_printoptions(formatter={'all':lambda x: "tests"})
        assert_equal(repr(x), "array(tests)")
        # str is unaffected
        assert_equal(str(x), "1")
    ...
    def test_floatmode(self):
        ...
        # tests unique special case (gh-18609)
...
class TestContextManager:
    def test_ctx_mgr(self):
        # tests that context manager actually works
        with np.printoptions(precision=2):
            s = str(np.array([2.0]) / 3)
        assert_equal(s, '[0.67]')

    def test_ctx_mgr_restores(self):
        # tests that print options are actually restored
    ...
    def test_ctx_mgr_exceptions(self):
        # tests that print options are restored even if an exception is raised
        opts = np.get_printoptions()
        try:
...
def test_multithreaded_array_printing():
    ...
    # reasons this tests makes sure it is set up in a thread-safe manner
