...
class TestAsIntegerRatio:
    # derived in part from the cpython tests "test_floatasratio"
    ...
    @pytest.mark.parametrize("ftype, frac_vals, exp_vals", [
        # dtype tests cases generated using hypothesis
...
class TestBitCount:
    # derived in part from the cpython tests "test_bit_count"

    @pytest.mark.parametrize("itype", sctypes['int']+sctypes['uint'])
    def test_small(self, itype):
        for a in range(max(np.iinfo(itype).min, 0), 128):
            msg = f"Smoke tests for {itype}({a}).bit_count()"
