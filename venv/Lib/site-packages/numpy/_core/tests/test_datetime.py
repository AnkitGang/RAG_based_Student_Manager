...
# Use pytz to tests out various time zones if available
try:
...
class TestDateTime:
    ...
    def test_datetime_prefix_conversions(self):
        ...
        # tests metric prefixes from seconds down to
    ...
    @pytest.mark.parametrize("op1, op2", [
        # reuse the tests cases from floordiv
        (np.timedelta64(7, 's'),
         np.timedelta64(4, 's')),
        # m8 same units round down with negative
        (np.timedelta64(7, 's'),
    ...
    def test_timedelta_correct_mean(self):
        # tests mainly because it worked only via a bug in that allowed:
