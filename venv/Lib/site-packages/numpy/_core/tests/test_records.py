...
class TestFromrecords:
    ...
    def test_zero_width_strings(self):
        # Test for #6430, based on the tests case from #1901

        cols = [['tests'] * 3, [''] * 3]
        rec = np.rec.fromarrays(cols)
        assert_equal(rec['f0'], ['tests', 'tests', 'tests'])
        assert_equal(rec['f1'], ['', '', ''])

        dt = np.dtype([('f0', '|S4'), ('f1', '|S')])
        rec = np.rec.fromarrays(cols, dtype=dt)
        assert_equal(rec.itemsize, 4)
        assert_equal(rec['f0'], [b'tests', b'tests', b'tests'])
...
class TestRecord:
    ...
    def test_nested_dtype_padding(self):
        """ tests that trailing padding is preserved """
