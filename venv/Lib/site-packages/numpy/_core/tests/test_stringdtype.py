...
def test_additional_unicode_cast(random_string_list, dtype):
    ...
    # tests that this short-circuits correctly
...
def test_comparisons(string_list, dtype, op, o_dtype):
    ...
    res = op(sarr, sarr)
    ores = op(oarr, oarr)
    # tests that promotion works as well
    orres = op(sarr, oarr)
    olres = op(oarr, sarr)
    ...
    # tests we get the correct answer for unequal length strings
...
def test_add_no_legacy_promote_with_signature():
    # Possibly misplaced, but useful to tests with string DType.  We check that
    # if there is clearly no loop found, a stray `dtype=` doesn't break things
    # Regression tests for the bad error in gh-26735
...
def test_threaded_access_and_mutation(dtype, random_string_list):
    # this tests uses an RNG and may crash or cause deadlocks if there is a
    # threading bug
...
def test_unset_na_coercion():
    ...
    # this tests uses the "add" and "equal" ufunc but all ufuncs that
...
def test_accumulation(string_array, tile):
    ...
    # Bloat size a bit (get above thresholds and tests >1 ndim).
