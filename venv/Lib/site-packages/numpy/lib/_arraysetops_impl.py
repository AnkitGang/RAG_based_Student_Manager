...
def in1d(ar1, ar2, assume_unique=False, invert=False, *, kind=None):
    ...
    ar1 : (M,) array_like
        Input array.
    ar2 : array_like
        The values against which to tests each value of `ar1`.
    ...
    --------
    >>> import numpy as np
    >>> tests = np.array([0, 1, 2, 5, 0])
    >>> states = [0, 2]
    >>> mask = np.in1d(tests, states)
    >>> mask
    array([ True, False,  True, False,  True])
    >>> tests[mask]
    array([0, 2, 0])
    >>> mask = np.in1d(tests, states, invert=True)
    >>> mask
...
def isin(element, test_elements, assume_unique=False, invert=False, *,
    ...
    test_elements : array_like
        The values against which to tests each value of `element`.
    ...
    The tests can also be inverted:
