...
# Generate some basic arrays to tests with.
scalars = [
    np.uint8,
    np.int8,
    np.uint16,
...
@pytest.mark.skipif(not IS_64BIT, reason="tests requires 64-bit system")
@pytest.mark.slow
@requires_memory(free_bytes=2 * 2**30)
def test_large_archive(tmpdir):
    # Regression tests for product of saving arrays with dimensions of array
