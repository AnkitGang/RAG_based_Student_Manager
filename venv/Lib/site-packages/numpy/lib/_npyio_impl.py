...
def save(file, arr, allow_pickle=True, fix_imports=np._NoValue):
    ...
    >>> with open('tests.npy', 'wb') as f:
    ...     np.save(f, np.array([1, 2]))
    ...     np.save(f, np.array([1, 3]))
    >>> with open('tests.npy', 'rb') as f:
    ...     a = np.load(f)
    ...     b = np.load(f)
    >>> print(a, b)
    # [1 2] [1 3]
...
def savetxt(fname, X, fmt='%.18e', delimiter=' ', newline='\n', header='',
    ...
    >>> np.savetxt('tests.out', x, delimiter=',')   # X is an array
    >>> np.savetxt('tests.out', (x,y,z))   # x,y,z equal sized 1D arrays
    >>> np.savetxt('tests.out', x, fmt='%1.4e')   # use exponential notation
    ...
    with fid_ctx:
        ...
        for (j, conv) in user_converters.items():
            ...
            if _is_string_like(j):
                ...
                i = j
            # Find the value to tests - first_line is not filtered by usecols:
            if len(first_line):
