"""
Pytest configuration and fixtures for the Numpy tests suite.
"""
import os
import string
import sys
...
# forces determinism and is designed for use via np.tests()
hypothesis.settings.register_profile(
    name="numpy-profile", deadline=None, print_blob=True,
)
hypothesis.settings.register_profile(
    name="np.tests() profile",
...
hypothesis.settings.load_profile(
    "numpy-profile" if os.path.isfile(_pytest_ini) else "np.tests() profile"
)
...
def pytest_addoption(parser):
    parser.addoption("--available-memory", action="store", default=None,
                     help=("Set amount of memory available for running the "
                           "tests suite. This can result to tests requiring "
...
def pytest_terminal_summary(terminalreporter, exitstatus, config):
    if NOGIL_BUILD and not gil_enabled_at_start and sys._is_gil_enabled():
        ...
        tr.line("This can happen with no tests failures if the RuntimeWarning")
        tr.line("raised by Python when this happens is filtered by a tests.")
        tr.line("")
...
def pytest_itemcollected(item):
    """
    Check FPU precision mode was not changed during tests collection.

    The clumsy way we do it here is mainly necessary because numpy
    still uses yield tests, which can execute code at tests collection
    time.
    """
...
def check_fpu_mode(request):
    """
    Check FPU precision mode was not changed during the tests.
    """
    ...
    if old_mode != new_mode:
        raise AssertionError("FPU precision mode changed from {0:#x} to {1:#x}"
                             " during the tests".format(old_mode, new_mode))

    collect_result = _collect_results.get(request.node)
    if collect_result is not None:
        old_mode, new_mode = collect_result
        raise AssertionError("FPU precision mode changed from {0:#x} to {1:#x}"
                             " when collecting the tests".format(old_mode,
                                                                new_mode))
