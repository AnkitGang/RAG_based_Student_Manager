...
class TestIntegral:
    ...
    def test_legint(self):
        ...
        # tests integration of zero polynomial
        for i in range(2, 5):
            k = [0]*(i - 2) + [1]
...
class TestFitting:
    ...
    def test_legfit(self):
        ...
        assert_almost_equal(wcoef2d, np.array([coef3, coef3]).T)
        # tests scaling with complex values x points whose square
        # is zero when summed.
        x = [1, 1j, -1, -1j]
        assert_almost_equal(leg.legfit(x, x, 1), [0, 1])
        assert_almost_equal(leg.legfit(x, x, [0, 1]), [0, 1])
        # tests fitting only even Legendre polynomials
        x = np.linspace(-1, 1)
        y = f2(x)
...
class TestGauss:
    ...
    def test_100(self):
        ...
        # tests orthogonality. Note that the results need to be normalized,
