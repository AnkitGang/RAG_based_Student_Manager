...
class WatchTests(unittest.TestCase):
    def setUp(self):
        # counter for a tests that needs tests global state
        self.callcount = 0
    ...
    def test_watch_with_decode(self):
        ...
        for e in w.stream(fake_api.get_namespaces):
            ...
            self.assertEqual("tests%d" % count, e['object'].metadata.name)
    ...
    def test_watch_with_interspersed_newlines(self):
        ...
        # Consume all tests events from the mock service, stopping when no more data is available.
        # Note that "timeout_seconds" below is not a timeout; rather, it disables retries and is
        # the only way to do so. Without that, the stream will re-read the tests data forever.
    ...
    def test_watch_with_multibyte_utf8(self):
        ...
        for event in w.stream(fake_api.get_configmaps, timeout_seconds=1):
            ...
            self.assertEqual("tests%d" % count, event['object'].metadata.name)
    ...
    def test_watch_with_invalid_utf8(self):
        ...
        fake_resp.release_conn = Mock()
        fake_resp.stream = Mock(
            # tests 1 uses 1 invalid utf-8 byte
            # tests 2 uses a sequence of 2 invalid utf-8 bytes
            # tests 3 uses a sequence of 3 invalid utf-8 bytes
            return_value=[
        ...
        for event in w.stream(fake_api.get_configmaps, timeout_seconds=1):
            ...
            self.assertEqual("tests%d" % count, event['object'].metadata.name)
            self.assertEqual("😄 %d" % count, event['object'].data["utf-8"])
            # expect N replacement characters in tests N
    ...
    def test_watch_resource_version_set(self):
        ...
        # more strict tests with worse error message
    ...
    def test_unmarshal_with_custom_object(self):
        ...
        event = w.unmarshal_event('{"type": "ADDED", "object": {"apiVersion":'
                                  '"tests.com/v1beta1","kind":"foo","metadata":'
...
#    Comment out the tests below, it does not work currently.
#    def test_watch_with_deserialize_param(self):
#        """tests watch.stream() deserialize param"""
#        # prepare tests data
...
#        fake_api.get_namespaces.__doc__ = ':return: V1NamespaceList'
#    
#        # tests case with deserialize=True
#        w = Watch()
...
#    
#        # tests case with deserialize=False
#        w = Watch()
