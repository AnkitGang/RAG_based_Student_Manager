...
collection_st = st.shared(
    strategies.collections(
        ...
        # Lower values make it more likely that a tests will trigger a persist to disk.
...
def test_sync_threshold(settings: Settings) -> None:
    ...
    collection = client.create_collection(
        name="tests", metadata={"hnsw:batch_size": 3, "hnsw:sync_threshold": 3}
    )
...
def get_multiprocessing_context():  # type: ignore[no-untyped-def]
    try:
        # Run the invariants in a new process to bypass any shared state/caching (which would defeat the purpose of the tests)
...
def test_batch_size_less_than_sync_with_duplicate_adds_results_in_skipped_seq_ids(
    ...
    # NOTE(hammadb) this tests was autogenerate by hypothesis and added here to ensure that the tests is run
