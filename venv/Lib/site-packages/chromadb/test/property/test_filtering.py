...
def _search_with_filter(
    ...
    """Use the search API to retrieve results with filters - tests helper function."""
    # Build Search object
    search = Search()
...
@example(
    ...
    record_set=strategies.RecordSet(
        ...
        metadatas=[{"tests": "tests"} for _ in range(11)],
        documents=None,
    ),
    filters=[
        strategies.Filter(
...
def test_empty_filter(client: ClientAPI) -> None:
    ...
    reset(client)
    coll = client.create_collection(name="tests")
...
def test_boolean_metadata(client: ClientAPI) -> None:
    ...
    test_ids: IDs = ["1", "2", "3"]
    test_embeddings: Embeddings = [np.array([1, 1]), np.array([2, 2]), np.array([3, 3])]
    test_metadatas: Metadatas = [{"tests": True}, {"tests": False}, {"tests": True}]

    coll.add(ids=test_ids, embeddings=test_embeddings, metadatas=test_metadatas)

    res = coll.get(where={"tests": True})
...
def test_get_empty(client: ClientAPI) -> None:
    ...
    test_ids: IDs = ["1", "2", "3"]
    test_embeddings: Embeddings = [np.array([1, 1]), np.array([2, 2]), np.array([3, 3])]
    test_metadatas: Metadatas = [{"tests": 10}, {"tests": 20}, {"tests": 30}]
    ...
    check_empty_res(res)
    res = coll.get(
        include=["embeddings", "metadatas", "documents"], where={"tests": 100}
    )
    check_empty_res(res)
...
def test_query_ids_filter_property(
    ...
    """Property tests for querying with only the ids filter."""
    if (
        client.get_settings().chroma_api_impl
        == "chromadb.api.async_fastapi.AsyncFastAPI"
    ):
        pytest.skip(
            "Skipping tests for async client due to potential resource/timeout issues"
        )
    caplog.set_level(logging.ERROR)
    reset(client)
    coll = client.create_collection(
...
def test_regex(client: ClientAPI) -> None:
    ...
    res = coll.get(
        where={"tests": {"$ne": 10}}, where_document={"$regex": "(?i)c(?-i)at"}  # type: ignore[dict-item]
    )
    assert res["ids"] == ["2"]
