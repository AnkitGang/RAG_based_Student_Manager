...
def _compute_expected_config_spann(
    ...
    # start off creating default spann config, we slowly modify it to much whatever prop tests provides
    ...
    if configuration:
        ...
        if spann_cfg:
            ...
            # update expected with whatever prop tests provides
...
def _extract_expected_schema_indexes(
    ...
    for key, value_types in schema.keys.items():
        if key in (EMBEDDING_KEY, "#document"):
            # Skip special keys - they're handled by vector index tests
            continue
