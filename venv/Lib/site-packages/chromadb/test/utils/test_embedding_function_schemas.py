...
def get_embedding_function_names() -> List[str]:
    """Get all embedding function names to tests"""
    return [
        name
...
class TestEmbeddingFunctionSchemas:
    ...
    def test_embedding_function_config_roundtrip(
        ...
        mock_call = MagicMock(return_value=mock_embeddings(["tests"]))
    ...
    def _setup_env_vars_for_ef(
        ...
        if ef_name in api_key_env_vars:
            mock_common_deps.setenv(api_key_env_vars[ef_name], "tests-api-key")

        # Special cases that need additional environment variables
        if ef_name == "amazon_bedrock":
            mock_common_deps.setenv("AWS_SECRET_ACCESS_KEY", "tests-secret-key")
    ...
    def _create_ef_instance(
        ...
        if ef_name == "cloudflare_workers_ai":
            ...
            return ef_class(project_id="tests-project", region="us-central1")
            ...
            try:
                ...
                return ef_class(model_name="tests-model")
