...
class _FakeClient:
    def __init__(self, collection_id: str = "tests-collection-id") -> None:
...
def test_wait_for_version_increase_logs_target_version(
    ...
    new_version = wait_module.wait_for_version_increase(client, "tests-collection", 5)

    assert new_version == 6
    assert (
        "[wait_for_version_increase] collection=tests-collection "
...
def test_wait_for_version_increase_timeout_mentions_waited_version(
    ...
    with pytest.raises(
        ...
        wait_module.wait_for_version_increase(client, "tests-collection", 8)
