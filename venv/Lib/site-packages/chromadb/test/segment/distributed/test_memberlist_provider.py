...
def update_memberlist(n: int, memberlist_name: str = "tests-memberlist") -> Memberlist:
    config.load_config()
    api_instance = client.CustomObjectsApi()

    members = [
        Member(id=f"tests-{i}", ip=f"10.0.0.{i}", node="node-{i}")
...
def test_can_get_memberlist() -> None:
    # This tests assumes that the memberlist CRD is already created with the name "tests-memberlist"
    system = System(Settings(allow_reset=True))
    provider = system.instance(CustomResourceMemberlistProvider)
    provider.set_memberlist_name("tests-memberlist")
    system.reset_state()
    system.start()
