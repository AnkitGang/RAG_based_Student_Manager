...
def test_does_not_affect_other_databases(client_factories: ClientFactories) -> None:
    ...
    first_client.create_collection("tests")

    second_client = client_factories.create_client(database="second")
    second_collection = second_client.create_collection("tests")

    admin_client.delete_database("first")

    assert second_client.get_collection("tests").id == second_collection.id

    with pytest.raises(NotFoundError):
        first_client.get_collection("tests")
