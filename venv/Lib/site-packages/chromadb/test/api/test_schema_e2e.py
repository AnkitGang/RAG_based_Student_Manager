...
def test_sparse_vector_source_key_and_index_constraints(
    ...
    sparse_ef = DeterministicSparseEmbeddingFunction(label="source-tests")
...
def test_sparse_auto_embedding_with_document_source_no_metadata(
    ...
    collection.add(
        ids=["doc1", "doc2", "doc3"],
        documents=["hello world", "tests document", "short"],
    )
    ...
    expected_batch = sparse_ef(["hello world", "tests document", "short"])
...
def test_sparse_auto_embedding_with_document_source_and_metadata(
    ...
    assert result["metadatas"][0]["category"] == "tests"
...
def test_create_index_with_key_type(
    ...
    assert set(str_result["ids"]) == {"key-tests-1"}

    # Test int field filter
    int_result = collection.get(where={"field_key": 42})
    assert set(int_result["ids"]) == {"key-tests-1"}
...
def test_search_api_uses_embedded_searches_with_sparse_embeddings(
    ...
    collection.add(
        ids=["doc1", "doc2"],
        documents=["hello world", "tests document"],
    )
...
def test_fts_disabled_schema_persistence(
    ...
    collection.add(
        ids=["persist-fts-1"],
        documents=["persistent doc for fts tests"],
    )
