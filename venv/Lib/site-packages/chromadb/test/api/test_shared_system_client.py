...
def clear_cache() -> Generator[None, None, None]:
    """Automatically clear the system cache before and after each tests."""
...
def test_extracts_api_key_from_chroma_cloud_client() -> None:
    mock_server_api = create_mock_http_client(
        ...
        headers={"X-Chroma-Token": "tests-api-key-123"},
    )
    register_mock_system("tests-id", mock_server_api)

    api_key = SharedSystemClient.get_chroma_cloud_api_key_from_clients()

    assert api_key == "tests-api-key-123"


def test_extracts_api_key_with_lowercase_header() -> None:
    mock_server_api = create_mock_http_client(
        api_url="https://api.trychroma.com/api/v2",
        headers={"x-chroma-token": "tests-api-key-456"},
    )
    register_mock_system("tests-id", mock_server_api)

    api_key = SharedSystemClient.get_chroma_cloud_api_key_from_clients()

    assert api_key == "tests-api-key-456"


def test_extracts_api_key_from_gcp_chroma_cloud_client() -> None:
    mock_server_api = create_mock_http_client(
        api_url="https://dummy.gcp.trychroma.com/api/v2",
        headers={"X-Chroma-Token": "gcp-tests-api-key"},
    )
    register_mock_system("tests-id", mock_server_api)

    api_key = SharedSystemClient.get_chroma_cloud_api_key_from_clients()

    assert api_key == "gcp-tests-api-key"
...
def test_skips_clients_without_api_url() -> None:
    mock_server_api = create_mock_http_client(
        api_url=None,
        headers={"X-Chroma-Token": "tests-api-key"},
    )
    register_mock_system("tests-id", mock_server_api)
...
def test_returns_first_api_key_found_from_multiple_clients() -> None:
    ...
    register_mock_system("tests-id-1", mock_server_api_1)
    register_mock_system("tests-id-2", mock_server_api_2)
...
def test_handles_exception_gracefully() -> None:
    ...
    SharedSystemClient._identifier_to_system["tests-id"] = mock_system
