...
class TestNewSchema:
    ...
    def test_many_keys_stress(self) -> None:
        """Test schema with many key overrides (stress tests)."""
        schema = Schema()
    ...
    def test_cmek_basic_creation(self) -> None:
        ...
        cmek = Cmek.gcp(
            "projects/tests-project/locations/us-central1/keyRings/tests-ring/cryptoKeys/tests-key"
        )
    ...
    def test_cmek_in_schema(self) -> None:
        ...
        cmek = Cmek.gcp("projects/tests/locations/us/keyRings/ring/cryptoKeys/key")
        ...
        assert (
            ...
            == "projects/tests/locations/us/keyRings/ring/cryptoKeys/key"
        )
...
def test_vector_index_config_source_key_accepts_key_type() -> None:
    ...
    config4 = VectorIndexConfig(source_key="tests")
    config5 = VectorIndexConfig(source_key=Key("tests"))  # type: ignore[arg-type]
...
def test_sparse_vector_index_config_source_key_accepts_key_type() -> None:
    ...
    # Test that both approaches produce the same result
    config4 = SparseVectorIndexConfig(source_key="tests")
    config5 = SparseVectorIndexConfig(source_key=Key("tests"))  # type: ignore[arg-type]
    assert config4.source_key == config5.source_key
...
def test_config_classes_reject_invalid_fields() -> None:
    ...
    with pytest.raises((ValueError, ValidationError)) as exc_info:
        VectorIndexConfig(invalid_field="tests")  # type: ignore[call-arg]
    ...
    with pytest.raises((ValueError, ValidationError)) as exc_info:
        FtsIndexConfig(invalid_field="tests")  # type: ignore[call-arg]
    ...
    with pytest.raises((ValueError, ValidationError)) as exc_info:
        StringInvertedIndexConfig(invalid_field="tests")  # type: ignore[call-arg]
