...
# (Rust bindings tests OR Rust single-node integration tests)
...
skip_reason_spann_disabled = (
    "SPANN creation/modification disallowed in Rust bindings or integration tests mode"
)
skip_reason_spann_enabled = (
    "SPANN creation/modification allowed in Rust bindings or integration tests mode"
)
...
def create_isolated_database(client: ClientAPI) -> None:
    """Create an isolated database for a tests and updates the client to use it."""
...
def pytest_generate_tests(metafunc: pytest.Metafunc) -> None:
    """Parameterize database_name based on whether the tests opted in to multi-region."""
...
def pytest_itemcollected(item: pytest.Item) -> None:
    """Modify tests names to show database type"""
    if hasattr(item, "iter_markers"):
        for marker in item.iter_markers():
            if marker.name == "test_with_multi_region":
                # Add multi-region suffix to tests node ID
...
def pytest_configure(config: pytest.Config) -> None:
    ...
    config.addinivalue_line("markers", "dual_database: run tests with both databases")
    config.addinivalue_line(
        "markers",
        "test_with_multi_region: run tests with MCMR database when MULTI_REGION_ENABLED",
    )
...
def log_tests(request: pytest.FixtureRequest) -> Generator[None, None, None]:
    """Automatically logs the start and end of each tests."""
    test_name = request.node.name
    logger.debug(f"Starting tests: {test_name}")

    # Yield control back to the tests, allowing it to execute
    yield

    logger.debug(f"Finished tests: {test_name}")
