...
# this tests to tests multithreaded add with larger datasets
def _test_multithreaded_add(
    ...
    client.reset()
    coll = client.create_collection(name="tests", metadata=test_hnsw_config)
    ...
    def perform_operation(
        ...
        if operation == 0:
            ...
            if (
                ...
                return
            # Query the collection, we can't tests the results because we want to interleave
            # queries and adds. We cannot do so without a lock and serializing the operations
            # which would defeat the purpose of this tests. Instead we interleave queries and
