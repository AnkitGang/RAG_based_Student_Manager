...
def test_persist_index_loading_embedding_function(api_fixture, request):
    ...
    client = request.getfixturevalue("local_persist_api")
    client.reset()
    collection = client.create_collection("tests", embedding_function=TestEF())
    collection.add(ids="id1", documents="hello")

    client2 = request.getfixturevalue("local_persist_api_cache_bust")
    collection = client2.get_collection("tests", embedding_function=TestEF())
...
def test_persist_index_get_or_create_embedding_function(api_fixture, request):
    ...
    api.reset()
    collection = api.get_or_create_collection("tests", embedding_function=TestEF())
    collection.add(ids="id1", documents="hello")

    api2 = request.getfixturevalue("local_persist_api_cache_bust")
    collection = api2.get_or_create_collection("tests", embedding_function=TestEF())
...
def test_collection_query_with_invalid_collection_throws(client):
    ...
    with pytest.raises(NotFoundError, match=r"Collection .* does not exist"):
        collection.query(query_texts=["tests"])
...
def test_collection_update_with_invalid_collection_throws(client):
    ...
    with pytest.raises(NotFoundError, match=r"Collection .* does not exist"):
        collection.update(ids=["id1"], documents=["tests"])
...
def test_where_valid_operators(client):
    ...
    with pytest.raises(ValueError):
        collection.get(where={"$contains": "tests"})
...
# tests to make sure add, get, delete error on invalid id input
...
# tests default embedding function
def test_default_embedding():
    embedding_function = DefaultEmbeddingFunction()
    docs = ["this is a tests" for _ in range(64)]
...
# tests to make sure add, query, update, upsert error on invalid embeddings input
...
# tests to make sure update shows exception for bad dimensionality
...
# tests to make sure upsert shows exception for bad dimensionality
...
def test_ssl_self_signed(client_ssl):
    if os.environ.get("CHROMA_INTEGRATION_TEST_ONLY"):
        pytest.skip("Skipping tests for integration tests")
...
def test_query_id_filtering_e2e(client):
    ...
    # tests deleted id filter raises
...
def test_list_metadata_validation():
    ...
    validate_metadata({"tags": ["a", "b"], "count": 5, "name": "tests"})
...
def test_array_metadata_e2e(client):
    """End-to-end tests: write array metadata, read it back, and query with $contains."""
...
def test_search_result_rows() -> None:
    ...
    # Should work as dict
    assert result["ids"] == [["tests"]]
    assert result.get("documents") == [["tests doc"]]
...
class TestWhereFromDict:
    ...
    def test_string_operators(self):
        ...
        # $regex
        where = Where.from_dict({"text": {"$regex": "^tests.*"}})
