...
def test_legacy_embedding_function(client: ClientAPI) -> None:
    ...
    coll2.add(ids=["1"], documents=["tests"])
    results = coll2.query(query_texts=["tests"], n_results=1)
...
def test_collection_load_with_invalid_configuration(client: ClientAPI) -> None:
    ...
    with pytest.raises(ValueError):
        coll.add(ids=["1"], documents=["tests"])

    with pytest.raises(ValueError):
        coll.query(query_texts=["tests"], n_results=1)
...
def test_invalid_configuration_operations_succeed_until_embed(
    ...
    for i, invalid_config in enumerate(invalid_configs):
        ...
        with pytest.raises(ValueError):
            coll.upsert(ids=["1"], documents=["tests"])
...
def test_get_collection_with_invalid_configuration(client: ClientAPI) -> None:
    ...
    with pytest.raises(ValueError):
        valid_coll.add(ids=["tests"], documents=["tests doc"])

    with pytest.raises(ValueError):
        valid_coll.query(query_texts=["tests"], n_results=1)

    with pytest.raises(ValueError):
        valid_coll.upsert(ids=["tests"], documents=["tests doc"])
